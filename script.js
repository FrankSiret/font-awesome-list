...
const faElem = document.querySelector(".font-awesome-list");

// fontAwesome.forEach(item_class => {

// 	let [type, name] = item_class.split(' ')
...
// 	li.setAttribute("class", "dib ma0 pb3 w-grid-2 w-grid-4-m w-grid-6-l w-grid-9-xl")

// 	const inner = `<div class="flex flex-column justify-center relative shadow-hover default hover-bg-white br2 hide-child">
// 	  <a class="flex flex-column items-center justify-center color-inherit w-100 pa2 br2 br--top no-underline hover-bg-blue4 hover-white">
// 	    <i class="${item_class}"></i>
// 	  </a>
// 	  <div class="w-100 ph1 pv2 tc f2">
// 	    <span class="db gray5 hover-gray7 text select-all">${type} ${name}</span>
// 	  </div>
// 	</div>`

// 	li.innerHTML = inner;

// 	faElem.insertAdjacentElement('beforeend', li);
// })

const search_query = document.getElementById('search_query');
const query_count = document.getElementById('query-count');
...
search_query.addEventListener("keyup", (e) => {
	renderResultDebounce(e.target.value)
})
...
const renderResult = (query) => {
	...
	const res = fontAwesome.filter(fa => {

		let [type, name] = fa.split(' ')
		...
		const temp = [type, name].join(' ')

		let match = true;

		query.split(' ').forEach(q => {
			if ( q && q.length )
				match = match && temp.match(`.*${q}.*`)
		})
		...
		const inner = `<li class="dib ma0 pb3 w-grid-2 w-grid-4-m w-grid-6-l w-grid-9-xl">
		  <div class="flex flex-column justify-center relative shadow-hover default hover-bg-white br2 hide-child">
		    <a class="flex flex-column items-center justify-center color-inherit w-100 pa2 br2 br--top no-underline hover-bg-blue4 hover-white">
		      <i class="${fa}"></i>
		    </a>
		    <div class="w-100 ph1 pv2 tc f2">
		      <span class="db gray5 hover-gray7 text select-all">${name}</span>
		    </div>
		  </div>
		</li>`
	...
	query_count.innerText = res.length == fontAwesome.length ? `All ${ res.length }` : `${ res.length } Resulting`
	faElem.innerHTML = res.join('');
}

const renderResultDebounce = debounce(renderResult, 500)

query_count.innerText = `All ${ fontAwesome.length }`

renderResultDebounce('')